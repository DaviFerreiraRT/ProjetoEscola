...
public class ProfessorDAO {
	...
	public static Professor create(String nome, String sexo) {
		...
		try {
			PreparedStatement ps = connection.prepareStatement("INSERT INTO PROFESSOR(NOME,SEXO) VALUES (?,?);",
	...
	public static Professor findById(int id) {
		Connection connection = Database.getConnection();
		try {
			PreparedStatement ps = connection.prepareStatement("SELECT * FROM PROFESSOR WHERE ID = ?");
	...
	public static void update(int id, String nome, String sexo) {
		...
		try {
			...
			if (exec > 0) {
				...
				JOptionPane.showMessageDialog(null, "Não existe nenhum professor" + " com este ID");
			}
